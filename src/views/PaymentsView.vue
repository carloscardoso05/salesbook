...
const editAmountCents = ref<number | null>(null)
...
function closeForm(): void {
  ...
  if (route.query.customerId) void router.replace({ query: {} })
}

watch(
  [customers, () => route.query.customerId],
  ([list, preselect]) => {
    if (typeof preselect === 'string' && list.some((customer) => customer.id === preselect)) {
      selectedCustomerId.value = preselect
      isFormOpen.value = true
      return
    }
...
async function submit(): Promise<void> {
  ...
  try {
    await service.addPayment({
      ...
      amountCents: amountCents.value ?? 0,
    })
...
function openEdit(payment: PaymentDocType): void {
  ...
  editAmountCents.value = payment.amountCents
}
...
async function saveEdit(): Promise<void> {
  ...
  try {
    await service.updatePayment(editing.value.id, { amountCents: editAmountCents.value ?? 0 })
...
<template>
  ...
  <ul v-else class="space-y-2.5">
    <li v-for="payment in payments" :key="payment.id" class="card card-pad flex items-center gap-2">
      <div class="min-w-0 flex-1">
        ...
        <p class="mt-0.5 text-xs text-slate-500">{{ formatDateTime(payment.createdAt) }}</p>
      </div>
  ...
  <ModalDialog :open="isFormOpen" title="Novo pagamento" @close="closeForm">
